...
# try to have a term of the form v_{ijk} = V((x_i - x_k).(x_j-x_k))
class EqTransformerFlowSherryVariationsimpleVdot(FlowNet):
    def __init__(self, input_dim, embed_dim=128, activation=nn.ReLU()):
        super().__init__()
        self.input_dim = input_dim
        self.embed_dim = embed_dim
        ...
        self.scaler = nn.Sequential(
            nn.Linear(1 + 1, embed_dim), # one less imput
            self.activation,
            nn.Linear(embed_dim, 1)
        )
        self.edges = None
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        # 4)  Displacements  (x_i - x_j)  →  (B,P,P,P,D)                     #
        # ------------------------------------------------------------------ #
        diff_pos = x.unsqueeze(2) - x.unsqueeze(1)        # (B,P,P,D)
        diff_ij = diff_pos.unsqueeze(3).expand(-1, -1, -1, P, -1) # (B,P,P,P, D)
        diff_kj = - diff_pos.unsqueeze(1).expand(-1, P, -1, -1, -1) # (B,P,P,P,D)
        dot_ijk = torch.sum(diff_ij * diff_kj, dim=-1, keepdim=True)  # (B,P,P,P,1)

        dot_feat = torch.cat((dot_ijk, t4), dim=-1)  # (B,P,P,P,2)
        s = self.scaler(dot_feat)
        # diff    = diff_ij.unsqueeze(2).expand(-1, P, P, P, -1) # don't need it
        # ------------------------------------------------------------------ #
        # 5)  Contract:   Σ_{k,j}  w_{kj} · s_{ijk} · (x_i - x_j)            #
        # ------------------------------------------------------------------ #
        v = torch.einsum('bijc, bijkc, bijd -> bid', W, s, diff_pos)     # (B,P,D)
        # print("change in effect")
        # v = torch.einsum('bijc, bikjc, bikjd -> bid', W, s, diff)     # (B,P,D)
        return v
    # This is a standard trick to have more parrallelizable computations between independent batches.
